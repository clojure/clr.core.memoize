...
(deftest test-regression-cmemoize-5
  (testing "that the TTL doesn't bomb on race-condition"
    (try
      ...
      (is (= 1 1))
      (catch NullReferenceException npe                                            ;;; NullPointerException
        (is (= 1 0))))))

;;;(deftest test-regression-cmemoize-7                                               ;;; doesn't make sense for CLR
;;;  (testing "that a memoized function that throws a RTE continues to retry"
;;;    (let [doit (fn [] (throw (Exception. "Foo")))                                 ;;; RuntimeException
;;;          it   (lru doit)]
;;;      (is (= :RTE
;;;             (try
;;;               (it)
...
;;;                 (try
;;;                   (it)
;;;                   (catch NullReferenceException _                                     ;;; NullPointerException
;;;                     :NPE)
